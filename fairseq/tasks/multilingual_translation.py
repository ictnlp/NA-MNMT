...
mask_backward = open('mask/taylor_mask', 'rb')
selfmask = pickle.load(mask_backward)

def change(currentmask):
    change.selfmask = currentmask
    

def get_module():

    #res = ["encoder.embed_tokens.weight","decoder.embed_tokens.weight", 'decoder.embed_out']
    res = ['decoder.embed_out']
...
def hook_fn_forward(module, input, output):
    ...
    if isinstance(input, tuple):
        ...
        #print(change.currentmask[module.__name__].size())
        #print(input[0].data.size())
        input[0].data.masked_fill_(change.currentmask[module.__name__].cuda().byte().bool(), 0.)
    else:
        input.data.masked_fill_(change.currentmask[module.__name__].cuda().byte().bool(), 0.)

    
def hook_fn_backward(module, grad_input, grad_output):
    #print(module.__name__) # 为了区分模块
    # 打印 grad_output
    #print(module.__name__, grad_input[0].size(), change.currentmask_backward[module.__name__].size())
    #tt = [grad_input[i].size() for i in range(len(grad_input))]
    #print(module.__name__, tt)
    if isinstance(grad_input, tuple):
        grad_input[1].data.masked_fill_(change.currentmask_backward[module.__name__].cuda().byte().bool(), 0.)
    else:
        grad_input.data.masked_fill_(change.currentmask_backward[module.__name__].cuda().byte().bool(), 0.)
    ...
    def train_step(self, sample, model, criterion, optimizer, ignore_grad=False):
        ...
        for lang_pair in self.model_lang_pairs:
            if sample[lang_pair] is None or len(sample[lang_pair]) == 0:
                continue
            change(selfmask[lang_pair])
            #print(change.currentmask.keys())
